...
class OnsetStage {
    ...
    static final DSP.FILTER_6TH_COEFF[] IIR_BAND_FILTERS = {
        // LP: 110
        new DSP.FILTER_6TH_COEFF(new double[] {
...
//        }, 4)
    ...
    static final int NUM_BANDS = IIR_BAND_FILTERS.length;
    ...
    public void createOnsetStream(AudioStream in, DataStream out, DataStream internal) {
        ...
        for (int i = NUM_BANDS - 1; i >= 0; i--) {
            DSP.DF2_Filter6(in, bandInputs[i], IIR_BAND_FILTERS[i], true);
            onsetDetect.createOnsetStream(bandInputs[i], onsets[i], internal);
        }

        // Reassemble onset streams into one conglomerate stream
        reassembleOnsets(out);
    }

    // Split the input signal into its separate frequency bands
    protected void bandSplitInput(AudioStream in) {
    }

    // Reassemble onset streams into one output stream
    protected void reassembleOnsets(DataStream out) {
